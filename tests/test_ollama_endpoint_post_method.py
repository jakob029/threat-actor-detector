...
class TestAnalyzisPost(unittest.TestCase):
    ...
    def test_post_success(self, mock_llama_json_parser, mock_correct_structure, mock_add_message, mock_set_graph, mock_send_prompt):
    ...
    def test_post_connect_timeout(self, mock_send_prompt):
        ...
        self.assertEqual(response.status_code, 500)
        self.assertEqual(response.json, {"message": "LLM_error"})
        mock_send_prompt.assert_called_once_with("example_prompt", "test_cid")
    ...
    def test_post_type_error(self, mock_send_prompt):
        ...
        self.assertEqual(response.status_code, 500)
        self.assertEqual(response.json, {"message": "parse error"})
        mock_send_prompt.assert_called_once_with("example_prompt", "test_cid")
